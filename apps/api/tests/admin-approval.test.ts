  ...
  prisma: {
    user: {
      findUnique: vi.fn().mockImplementation(({ where }) => {
        ...
        if (where.id === 'admin-test-id-789') {
          return Promise.resolve({ id: 'admin-test-id-789', email: 'admin@example.com', role: 'ADMIN', status: 'ACTIVE', name: 'Admin' });
        }
...
describe('Admin Approval Workflow & RBAC Rules', () => {
  ...
  it('RULE 2: Admin rejection MUST require a rejection reason of at least 5 characters', async () => {
    const res = await request(app)
